...
namespace Service_TechCompass.Services
{
    public class StudentProfileService : IStudentProfileService
    {
        ...
        public Task<(int StatusCode, string Message, object? Data)> GetProfileAsync(Guid userId)
        {
            var user = _userRepo.GetUserById(userId);
            if (user == null)
            {
                return Task.FromResult<(int, string, object?)>((404, "Không tìm thấy tài khoản người dùng.", null));
            }
            ...
            {
                case 1: // Admin
                    return Task.FromResult<(int, string, object?)>((200, "Lấy thông tin Admin thành công.", new
                    {
                        User = baseProfile,
                ...
                case 2: // Student
                    ...
                    if (student == null) return Task.FromResult<(int, string, object?)>((404, "Không tìm thấy hồ sơ sinh viên.", null));

                    return Task.FromResult<(int, string, object?)>((200, "Lấy thông tin Sinh viên thành công.", new
                    {
                        User = baseProfile,
                        Details = new
                        {
                ...
                case 3: // Mentor
                    var mentor = _userRepo.GetMentorByUserId(userId);
                    if (mentor == null) return Task.FromResult<(int, string, object?)>((404, "Không tìm thấy hồ sơ Mentor.", null));

                    return Task.FromResult<(int, string, object?)>((200, "Lấy thông tin Mentor thành công.", new
                    {
                        User = baseProfile,
                        Details = new
                        {
                            FullName = mentor.FullName,
                            // ĐÃ SỬA: Dùng đúng các trường có trong Model Mentor
                ...
                case 4: // Counselor
                    var counselor = _userRepo.GetCounselorByUserId(userId);
                    if (counselor == null) return Task.FromResult<(int, string, object?)>((404, "Không tìm thấy hồ sơ Counselor.", null));

                    return Task.FromResult<(int, string, object?)>((200, "Lấy thông tin Counselor thành công.", new
                    {
                        User = baseProfile,
                        Details = new
                        {
                ...
                default:
                    return Task.FromResult<(int, string, object?)>((200, "Lấy thông tin thành công.", new { User = baseProfile }));
            }
        }
